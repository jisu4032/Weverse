...
이메일 인증코드를 직접 입력하는 형식으로 구현하였습니다 테스트 실행중 터미널 입력이 필요하므로 `-s` 옵션을 반드시 포함후 실행 부탁드립니다.
...
- 이메일 인증코드는 현재 테스트 실행 중 터미널에서 수동으로 입력하는 방식으로 구현했습니다. 자동화 관점에서 한계가 있다는 점을 인지하고 있으며, IMAP 프로토콜을 활용해 이메일을 직접 파싱하고 인증코드를 추출하는 방식으로 개선하고 싶습니다. 추후 IMAP 연동을 직접 구현해보겠습니다.
